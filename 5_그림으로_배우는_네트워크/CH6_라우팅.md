...
- ![라우터와 연결된 모습](https://broadbandlibrary.com/wp-content/uploads/2018/08/Digram1.800.jpg)
...
- 복수의 스위치에 걸쳐 VLAN을 구성할 수 있으나 스위치 간의 연결이 필요함
...
- VLAN간 라우팅을 실현하려면 라우터 또는 레이어3 스위치가 필요, 레이어3가 더 효율적인 VLAN간 라우팅
- IP주소를 설정해야 네트워크(VLAN)가 연결됨
...
- 직접 접속된 네트워크의 경로 정보가 라우팅 테이블에 등록
- PC 자신이 연결되어 있는 네트워크 이외는 전부 '0.0.0.0/0'인 디폴트 경로로 모아서 등록.
- 그 설정이 **기본 게이트웨이의 IP 주소** 설정